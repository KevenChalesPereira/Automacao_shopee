...
def render(job_dir, product, background, product_png, audio, captions_ass, work, creator_video=None, creator_seconds=4.0):
    ...
    if has_creator:
        ...
        pill1_enable = f"between(t,{creator_seconds+0.35:.3f},{creator_seconds+1.95:.3f})"
        pill2_enable = f"between(t,{creator_seconds+2.15:.3f},{creator_seconds+3.75:.3f})"
        pill3_enable = f"between(t,{creator_seconds+3.95:.3f},{creator_seconds+5.55:.3f})"
    elif has_video:
        hook_enable = "between(t,0,2.95)"
...
[2:v]format=rgba[hook];[v1b][hook]overlay=x='(W-w)/2':y='if(lt(t,0.3),-h+(t/0.3)*(84+h),84)':enable='{hook_enable}':eval=frame[v2];
...
def main():
    if len(sys.argv)!=3:
        raise SystemExit('Uso: render_v20.py JOB_DIR OUTPUT_DIR')
