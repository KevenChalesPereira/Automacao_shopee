...
def make_visual_assets(work, product):
    ...
    # V22: hook pequeno, sem ocupar o topo inteiro.
    ...
    # CTA final curto para anúncio Shopee.
...
def make_shopee_ass(srt, ass_path):
    cues=parse_srt(srt)
...
def main():
    if len(sys.argv)!=3:
        raise SystemExit('Uso: render_v22.py JOB_DIR OUTPUT_DIR')
    ...
    # V22: Creator IA está pausado para não bloquear nem atrasar o anúncio Shopee.
    creator_video = None
    creator_info = {'ok': False, 'engine': 'disabled_v22', 'note': 'Pausado para priorizar estabilidade.'}
    product['creator_status'] = 'disabled_v22'
    product['creator_engine'] = 'disabled_v22'
    product['creator_info'] = creator_info
    final=render(job_dir,product,background,product_png,audio,ass,work,creator_video=None,creator_seconds=0)
    ...
    shutil.copy2(ass,output_dir/'legendas_shopee.ass')
    ...
    diagnostic = {
        ...
        'render_mode': 'raw_product_video' if int(product.get('quantidade_videos_apoio') or 0) else 'image_motion',
