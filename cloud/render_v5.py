...
[1:v]format=rgba,scale=790:-1,fade=t=in:st=0:d=0.30:alpha=1[p];
...
def main():
    if len(sys.argv)!=3:
        raise SystemExit('Uso: render_v5.py JOB_DIR OUTPUT_DIR')
    ...
    run(['edge-tts','--file',str(roteiro),'--voice',VOICE,'--rate=+5%','--write-media',str(audio),'--write-subtitles',str(srt)])
    ...
    (output_dir/'copie_e_cole_na_live.txt').write_text(
        ...
        encoding='utf-8')
    print('[OK] Render V5.1 comercial concluído.',flush=True)
