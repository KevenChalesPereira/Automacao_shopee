...
def make_visual_assets(work, product):
    ...
    hook_text = str(product.get('hook_visual') or 'ACHADO QUE CHAMA ATENÇÃO').upper()
    title = str(product.get('titulo_curto') or product.get('nome_limpo') or 'ACHADO SHOPEE').upper()
    theme = str(product.get('visual_theme') or 'marketplace').lower()

    bubble = Image.new('RGBA', (960, 260), (0, 0, 0, 0))
    d = ImageDraw.Draw(bubble, 'RGBA')
    d.rounded_rectangle((12, 10, 948, 224), radius=48, fill=(7, 10, 20, 235), outline=(255, 255, 255, 18), width=2)
    d.polygon([(420, 224), (540, 224), (485, 255)], fill=(7, 10, 20, 235))
    wrap = '\n'.join(textwrap.wrap(hook_text, width=26, break_long_words=False))
    f = fit_font(d, wrap, 830, 56, 32)
    d.multiline_text((480, 112), wrap, anchor='mm', align='center', font=f, fill=(255, 255, 255, 255), spacing=3)
    bubble.save(work / 'hook.png')

    sticker = Image.new('RGBA', (320, 320), (0, 0, 0, 0))
    sd = ImageDraw.Draw(sticker, 'RGBA')
    cx, cy = 160, 160
    burst = []
    for i in range(20):
        ang = math.pi * 2 * i / 20.0
        rr = 138 if i % 2 == 0 else 92
        burst.append((cx + rr * math.cos(ang), cy + rr * math.sin(ang)))
    sd.polygon(burst, fill=(255, 101, 51, 250), outline=(255, 245, 236, 255))
    sd.ellipse((54, 54, 266, 266), fill=(255, 244, 235, 255), outline=(255, 255, 255, 255), width=5)
    badge_text = str(product.get('badge_text') or 'ACHADO DO DIA').upper()
    badge_lines = badge_text.split()
    if len(badge_lines) >= 4:
        top_line = ' '.join(badge_lines[:2])
        bottom_line = ' '.join(badge_lines[2:])
    elif len(badge_lines) == 3:
        top_line = ' '.join(badge_lines[:2])
        bottom_line = badge_lines[2]
    elif len(badge_lines) == 2:
        top_line, bottom_line = badge_lines
    else:
        top_line, bottom_line = 'ACHADO', badge_text
    sd.text((160, 122), top_line, anchor='mm', font=font(30), fill=(255, 94, 42, 255))
    sd.text((160, 170), bottom_line, anchor='mm', font=font(34), fill=(24, 26, 31, 255))
    sticker = sticker.rotate(-10, resample=Image.Resampling.BICUBIC, expand=True)
    sticker.save(work / 'sticker.png')

    arrow = Image.new('RGBA', (460, 300), (0, 0, 0, 0))
    ad = ImageDraw.Draw(arrow, 'RGBA')
    ad.arc((30, 20, 410, 260), start=220, end=350, fill=(255, 255, 255, 220), width=10)
    ad.line((330, 220, 380, 190), fill=(255, 255, 255, 220), width=10)
    ad.line((330, 220, 370, 245), fill=(255, 255, 255, 220), width=10)
    for sx, sy, rr in [(76, 56, 18), (106, 118, 12), (130, 84, 8)]:
        ad.line((sx - rr, sy, sx + rr, sy), fill=(255, 104, 55, 235), width=4)
        ad.line((sx, sy - rr, sx, sy + rr), fill=(255, 104, 55, 235), width=4)
    arrow.save(work / 'arrow.png')

    board = Image.new('RGBA', (980, 370), (0, 0, 0, 0))
    bd_shadow = Image.new('RGBA', board.size, (0, 0, 0, 0))
    bsd = ImageDraw.Draw(bd_shadow, 'RGBA')
    bsd.rounded_rectangle((22, 24, 960, 352), radius=58, fill=(0, 0, 0, 120))
    bd_shadow = bd_shadow.filter(ImageFilter.GaussianBlur(24))
    board.alpha_composite(bd_shadow)
    bd = ImageDraw.Draw(board, 'RGBA')
    bd.rounded_rectangle((10, 12, 970, 340), radius=58, fill=(247, 244, 239, 247))
    tf = _fit_single_line(bd, title, 820, 62, 34)
    bd.text((490, 66), title, anchor='mm', font=tf, fill=(22, 25, 30, 255))
    yy = 126
    for pt in points:
        bd.ellipse((95, yy - 18, 141, yy + 28), fill=(49, 155, 88, 255))
        bd.text((118, yy + 4), '✓', anchor='mm', font=font(28), fill=(255, 255, 255, 255))
        pf = _fit_single_line(bd, str(pt).upper(), 650, 42, 24)
        bd.text((185, yy + 4), str(pt).upper(), anchor='lm', font=pf, fill=(28, 31, 36, 255))
        yy += 62
    board.save(work / 'board.png')

    # V17: cards compactos para vídeos reais. Entram um por vez e não escondem o produto.
    for idx, pt in enumerate(points[:3], start=1):
        pill = Image.new('RGBA', (720, 112), (0,0,0,0))
        pd = ImageDraw.Draw(pill, 'RGBA')
        shadow = Image.new('RGBA', pill.size, (0,0,0,0))
        shd = ImageDraw.Draw(shadow, 'RGBA')
        shd.rounded_rectangle((18,22,704,102), radius=34, fill=(0,0,0,110))
        shadow = shadow.filter(ImageFilter.GaussianBlur(13))
        pill.alpha_composite(shadow)
        pd.rounded_rectangle((8,8,712,96), radius=34, fill=(10,14,24,225), outline=(255,255,255,36), width=2)
        pd.ellipse((28,27,78,77), fill=(255,101,51,255))
        pd.text((53,52), str(idx), anchor='mm', font=font(27), fill=(255,255,255,255))
        pf=_fit_single_line(pd, str(pt).upper(), 570, 42, 24)
        pd.text((105,52), str(pt).upper(), anchor='lm', font=pf, fill=(255,255,255,255))
        pill.save(work / f'pill{idx}.png')

    cta = Image.new('RGBA', (850, 168), (0, 0, 0, 0))
    cshadow = Image.new('RGBA', cta.size, (0, 0, 0, 0))
    csd = ImageDraw.Draw(cshadow, 'RGBA')
    csd.rounded_rectangle((20, 30, 830, 150), radius=42, fill=(0, 0, 0, 110))
    cshadow = cshadow.filter(ImageFilter.GaussianBlur(16))
    cta.alpha_composite(cshadow)
    cd = ImageDraw.Draw(cta, 'RGBA')
    cd.rounded_rectangle((8, 12, 842, 144), radius=42, fill=(255, 92, 45, 255))
    cta_main = str(product.get('cta_main') or 'CONFIRA NA SHOPEE').upper()
    cd.text((425, 76), cta_main, anchor='mm', font=font(52), fill=(255, 255, 255, 255))
...
def render(job_dir, product, background, product_png, audio, captions_ass, work):
    ...
    has_real_media = has_video or has_images

    # Com vídeo real, a foto principal vira somente abertura + fechamento.
    if has_video:
        product_enable = f"between(t,0,2.75)+gte(t,{end_cta:.3f})"
        board_enable = "between(t,0,0.01)"
        pill1_enable = "between(t,4.1,6.3)"
        pill2_enable = "between(t,7.0,9.2)"
        pill3_enable = "between(t,10.0,12.2)"
        arrow_enable = "between(t,0,0.01)"
    elif has_images:
        product_enable = f"between(t,0,3.4)+gte(t,{end_cta:.3f})"
        board_enable = "between(t,5.0,7.8)"
        pill1_enable = "between(t,8.2,10.0)"
        pill2_enable = "between(t,10.3,12.1)"
        pill3_enable = "between(t,12.4,14.2)"
        arrow_enable = "between(t,1.0,3.4)"
    else:
        product_enable = '1'
        board_enable = f"between(t,5.1,{max(8.5,end_cta-0.4):.3f})"
        pill1_enable = pill2_enable = pill3_enable = "between(t,0,0.01)"
        arrow_enable = "between(t,1.0,5.0)"
    ...
    background_video = build_background_video(job_dir, bg_local, total, work)
    ...
    fc = f"""[0:v]scale=1080:1920,setsar=1,eq=brightness=-0.02:saturation=1.08,vignette=PI/5[bg0];
[8:v]format=rgba,colorchannelmixer=aa=0.52[flare];
[bg0][flare]overlay=x='(W-w)/2+4*sin(t*0.85)':y='450+6*cos(t*0.75)':eval=frame[bg];
[1:v]format=rgba,scale=820:-1,fade=t=in:st=0:d=0.22:alpha=1[p];
[bg][p]overlay=x='(W-w)/2+7*sin(t*1.1)':y='if(lt(t,0.45),940-(t/0.45)*600,330+7*sin(t*1.1))':enable='{product_enable}':eval=frame[v1];
[9:v]format=rgba,colorchannelmixer=aa=0[pres];
[v1][pres]overlay=x=0:y=0:enable='between(t,0,0.01)'[v1p];
[7:v]format=rgba[shine];[shine]split=2[s1][s2];
[v1p][s1]overlay=x='-w+(t-0.62)/0.76*(W+w)':y=0:enable='between(t,0.62,1.38)':eval=frame[v1a];
[v1a][s2]overlay=x='-w+(t-{end_cta:.3f})/0.76*(W+w)':y=0:enable='between(t,{end_cta:.3f},{end_cta+0.76:.3f})':eval=frame[v1b];
[2:v]format=rgba[hook];[v1b][hook]overlay=x='(W-w)/2':y='if(lt(t,0.32),-h+(t/0.32)*(80+h),80)':enable='between(t,0,3.45)':eval=frame[v2];
[3:v]format=rgba[sticker];[v2][sticker]overlay=x='88+6*sin(t*1.6)':y='350+6*cos(t*1.3)':enable='between(t,0.7,3.35)':eval=frame[v3];
[4:v]format=rgba[arrow];[v3][arrow]overlay=x='610+8*sin(t*0.9)':y='690+6*cos(t*1.1)':enable='{arrow_enable}':eval=frame[v4];
[5:v]format=rgba[board];[v4][board]overlay=x='(W-w)/2':y=1515:enable='{board_enable}'[v5];
[10:v]format=rgba[pill1];[v5][pill1]overlay=x='(W-w)/2':y=1290:enable='{pill1_enable}'[v6];
[11:v]format=rgba[pill2];[v6][pill2]overlay=x='(W-w)/2':y=1290:enable='{pill2_enable}'[v7];
[12:v]format=rgba[pill3];[v7][pill3]overlay=x='(W-w)/2':y=1290:enable='{pill3_enable}'[v8];
[6:v]format=rgba[cta];[v8][cta]overlay=x='(W-w)/2':y='if(lt(t,{end_cta+0.32:.3f}),H-(t-{end_cta:.3f})/0.32*(H-1760),1760)':enable='gte(t,{end_cta:.3f})'[v9];
[v9]ass=captions.ass[v];
[13:a]volume=1.12[narr];
[14:a]volume=0.13[musicbed];
...
""".replace('\n','')
    ...
    for img in [product_png, work/'hook.png', work/'sticker.png', work/'arrow.png', work/'board.png', work/'cta.png', work/'shine.png', work/'hero_flare.png', presenter_png, work/'pill1.png', work/'pill2.png', work/'pill3.png']:
        cmd += ['-loop', '1', '-framerate', '30', '-i', str(img)]
    cmd += ['-i', str(audio), '-i', str(music), '-filter_complex', fc, '-map', '[v]', '-map', '[aout]', '-t', f'{total:.3f}', '-r', '30', '-c:v', 'libx264', '-preset', 'veryfast', '-crf', '21', '-pix_fmt', 'yuv420p', '-c:a', 'aac', '-b:a', '160k', '-movflags', '+faststart', str(output)]
    run(cmd, cwd=work)
    return output

def main():
    if len(sys.argv)!=3:
        raise SystemExit('Uso: render_v17.py JOB_DIR OUTPUT_DIR')
    ...
    # presenter asset is optional and may be used by the render as lightweight UGC avatar
    if not (work/'presenter.png').exists():
        prepare_presenter(job_dir, work)
    final=render(job_dir,product,background,product_png,audio,ass,work)
    ...
    (output_dir/'copie_e_cole_na_live.txt').write_text(
        ...
        encoding='utf-8')
    print('[OK] Render V17 media-first concluído.',flush=True)
