...
def make_visual_assets(work, product):
    ...
    hook = Image.new('RGBA', (940, 220), (0, 0, 0, 0))
    hd = ImageDraw.Draw(hook, 'RGBA')
    shadow = Image.new('RGBA', hook.size, (0, 0, 0, 0))
    sd = ImageDraw.Draw(shadow, 'RGBA')
    sd.rounded_rectangle((18, 24, 922, 202), radius=44, fill=(0, 0, 0, 105))
    shadow = shadow.filter(ImageFilter.GaussianBlur(18))
    hook.alpha_composite(shadow)
    hd.rounded_rectangle((8, 8, 932, 192), radius=44, fill=(7, 10, 20, 236), outline=(255,255,255,18), width=2)
    wrap = '\n'.join(textwrap.wrap(hook_text, width=26, break_long_words=False))
    f = fit_font(hd, wrap, 820, 56, 30)
    hd.multiline_text((470, 98), wrap, anchor='mm', align='center', font=f, fill=(255,255,255,255), spacing=3)
    hook.save(work / 'hook.png')

    board = Image.new('RGBA', (960, 290), (0, 0, 0, 0))
    bs = Image.new('RGBA', board.size, (0, 0, 0, 0))
    bsd = ImageDraw.Draw(bs, 'RGBA')
    bsd.rounded_rectangle((16, 24, 944, 270), radius=48, fill=(0, 0, 0, 95))
    bs = bs.filter(ImageFilter.GaussianBlur(20))
    board.alpha_composite(bs)
    bd = ImageDraw.Draw(board, 'RGBA')
    bd.rounded_rectangle((8, 8, 952, 254), radius=48, fill=(248,246,242,248))
    tf = _fit_single_line(bd, title, 780, 56, 30)
    bd.text((480, 56), title, anchor='mm', font=tf, fill=(20,22,28,255))
    y = 118
    for pt in points[:2]:
        bd.ellipse((85, y-16, 125, y+24), fill=(47, 154, 88, 255))
        bd.text((105, y+4), '✓', anchor='mm', font=font(24), fill=(255,255,255,255))
        pf = _fit_single_line(bd, str(pt).upper(), 640, 38, 22)
        bd.text((155, y+3), str(pt).upper(), anchor='lm', font=pf, fill=(28,31,36,255))
        y += 58
    board.save(work / 'board.png')

    for idx, pt in enumerate(points[:3], start=1):
        pill = Image.new('RGBA', (760, 104), (0,0,0,0))
        pd = ImageDraw.Draw(pill, 'RGBA')
        sh = Image.new('RGBA', pill.size, (0,0,0,0))
        shd = ImageDraw.Draw(sh, 'RGBA')
        shd.rounded_rectangle((14,20,746,94), radius=32, fill=(0,0,0,92))
        sh = sh.filter(ImageFilter.GaussianBlur(14))
        pill.alpha_composite(sh)
        pd.rounded_rectangle((8,8,752,84), radius=32, fill=(10,14,24,212), outline=(255,255,255,28), width=2)
        pd.ellipse((24,22,66,64), fill=(255,101,51,255))
        pd.text((45,43), str(idx), anchor='mm', font=font(24), fill=(255,255,255,255))
        pf = _fit_single_line(pd, str(pt).upper(), 610, 38, 22)
        pd.text((92,44), str(pt).upper(), anchor='lm', font=pf, fill=(255,255,255,255))
        pill.save(work / f'pill{idx}.png')

    cta = Image.new('RGBA', (860, 150), (0, 0, 0, 0))
    cs = Image.new('RGBA', cta.size, (0,0,0,0))
    csd = ImageDraw.Draw(cs, 'RGBA')
    csd.rounded_rectangle((20, 24, 840, 136), radius=40, fill=(0,0,0,94))
    cs = cs.filter(ImageFilter.GaussianBlur(15))
    cta.alpha_composite(cs)
    cd = ImageDraw.Draw(cta, 'RGBA')
    cd.rounded_rectangle((8, 8, 852, 120), radius=40, fill=(255, 92, 45, 255))
    cta_main = str(product.get('cta_main') or 'CONFIRA NA SHOPEE').upper()
    cf = _fit_single_line(cd, cta_main, 700, 50, 30)
    cd.text((430, 64), cta_main, anchor='mm', font=cf, fill=(255,255,255,255))
...
def render(job_dir, product, background, product_png, audio, captions_ass, work):
    ...
    if has_video:
        product_enable = f"between(t,0,2.55)+gte(t,{end_cta:.3f})"
        board_enable = "between(t,0,0.01)"
        pill1_enable = "between(t,4.0,5.8)"
        pill2_enable = "between(t,6.4,8.2)"
        pill3_enable = "between(t,8.8,10.6)"
    elif has_images:
        product_enable = f"between(t,0,3.2)+gte(t,{end_cta:.3f})"
        board_enable = "between(t,3.2,6.6)"
        pill1_enable = "between(t,6.9,8.5)"
        pill2_enable = "between(t,8.8,10.4)"
        pill3_enable = "between(t,10.7,12.3)"
    else:
        product_enable = '1'
        board_enable = f"between(t,2.6,{max(6.8,end_cta-0.6):.3f})"
        pill1_enable = "between(t,6.8,8.4)"
        pill2_enable = "between(t,8.7,10.3)"
        pill3_enable = "between(t,10.6,12.2)"
...
[2:v]format=rgba[hook];[v1b][hook]overlay=x='(W-w)/2':y='if(lt(t,0.3),-h+(t/0.3)*(84+h),84)':enable='between(t,0,2.95)':eval=frame[v2];
[3:v]format=rgba[board];[v2][board]overlay=x='(W-w)/2':y=1490:enable='{board_enable}'[v3];
[8:v]format=rgba[pill1];[v3][pill1]overlay=x='(W-w)/2':y=1410:enable='{pill1_enable}'[v4];
[9:v]format=rgba[pill2];[v4][pill2]overlay=x='(W-w)/2':y=1410:enable='{pill2_enable}'[v5];
[10:v]format=rgba[pill3];[v5][pill3]overlay=x='(W-w)/2':y=1410:enable='{pill3_enable}'[v6];
[4:v]format=rgba[cta];[v6][cta]overlay=x='(W-w)/2':y='if(lt(t,{end_cta+0.34:.3f}),H-(t-{end_cta:.3f})/0.34*(H-1750),1750)':enable='gte(t,{end_cta:.3f})'[v7];
...
def main():
    if len(sys.argv)!=3:
        raise SystemExit('Uso: render_v19.py JOB_DIR OUTPUT_DIR')
    ...
    (output_dir/'copie_e_cole_na_live.txt').write_text(
        ...
        encoding='utf-8')
    print('[OK] Render V19 media-first concluído.',flush=True)
