...
def render(job_dir, product, background, product_png, audio, captions_ass, work):
    ...
    has_real_media = bool(product.get('quantidade_videos_apoio') or product.get('quantidade_imagens_apoio'))
    product_enable = f"between(t,0,4.7)+gte(t,{end_cta:.3f})" if has_real_media else '1'
    board_end = min(max(8.8, total * 0.55), max(9.0, end_cta - 0.35))
    board_enable = f"between(t,5.25,{board_end:.3f})"
...
[bg][p]overlay=x='(W-w)/2+8*sin(t*1.1)':y='if(lt(t,0.55),980-(t/0.55)*640,340+8*sin(t*1.1)+4*sin(t*2.4))':enable='{product_enable}':eval=frame[v1];
[9:v]format=rgba,colorchannelmixer=aa=0[pres];
[v1][pres]overlay=x=0:y=0:enable='between(t,0,0.01)':eval=frame[v1p];
...
[3:v]format=rgba[sticker];[v2][sticker]overlay=x='88+8*sin(t*1.6)':y='350+8*cos(t*1.3)':enable='between(t,0.85,5.8)':eval=frame[v3];
[4:v]format=rgba[arrow];[v3][arrow]overlay=x='610+8*sin(t*0.9)':y='690+6*cos(t*1.1)':enable='between(t,1.1,6.2)':eval=frame[v4];
[5:v]format=rgba[board];[v4][board]overlay=x='(W-w)/2':y='if(lt(t,5.9),H-(t-5.28)/0.62*(H-1535),1535)':enable='{board_enable}':eval=frame[v5];
[6:v]format=rgba[cta];[v5][cta]overlay=x='(W-w)/2':y='if(lt(t,{end_cta+0.35:.3f}),H-(t-{end_cta:.3f})/0.35*(H-1762),1762)':enable='gte(t,{end_cta:.3f})':eval=frame[v6];
...
def main():
    if len(sys.argv)!=3:
        raise SystemExit('Uso: render_v16.py JOB_DIR OUTPUT_DIR')
    ...
    (output_dir/'copie_e_cole_na_live.txt').write_text(
        ...
        encoding='utf-8')
    print('[OK] Render V16 marketing concluído.',flush=True)
