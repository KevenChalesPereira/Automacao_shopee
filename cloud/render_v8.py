...
def make_visual_assets(work, product):
    ...
    sticker_top = 'ACHADO'
    sticker_bottom = 'DO DIA'
    if theme == 'tech':
        sticker_bottom = 'SEM FIO'
    elif theme == 'wellness':
        sticker_bottom = 'DIA A DIA'
    elif theme == 'footwear':
        sticker_bottom = 'VISUAL'
    sd.text((160, 122), sticker_top, anchor='mm', font=font(34), fill=(255, 94, 42, 255))
    sd.text((160, 170), sticker_bottom, anchor='mm', font=font(38), fill=(24, 26, 31, 255))
    ...
    cd.text((425, 76), 'CONFIRA NA SHOPEE', anchor='mm', font=font(52), fill=(255, 255, 255, 255))
...
def make_tiktok_ass(srt, ass_path):
    cues=parse_srt(srt)
    header="""[Script Info]\nScriptType: v4.00+\nPlayResX: 1080\nPlayResY: 1920\nScaledBorderAndShadow: yes\nWrapStyle: 2\n\n[V4+ Styles]\nFormat: Name,Fontname,Fontsize,PrimaryColour,SecondaryColour,OutlineColour,BackColour,Bold,Italic,Underline,StrikeOut,ScaleX,ScaleY,Spacing,Angle,BorderStyle,Outline,Shadow,Alignment,MarginL,MarginR,MarginV,Encoding\nStyle: Pop,DejaVu Sans,62,&H00FFFFFF,&H005ED6FF,&H00000000,&H90000000,-1,0,0,0,100,100,0,0,1,8,3,2,48,48,250,1\n\n[Events]\nFormat: Layer,Start,End,Style,Name,MarginL,MarginR,MarginV,Effect,Text\n"""
    events=[]
    toggle=0
...
def make_marketing_banners(product, background, product_png, output_dir):
    ...
    prod = _hero_product(product_png, (980, 1120))
    ...
    sd.text((120, 110), 'ACHADO', anchor='mm', font=font(26), fill=(255,255,255,255))
    sd.text((120, 145), 'SHOPEE', anchor='mm', font=font(30), fill=(255,245,235,255))
    sticker = sticker.rotate(-12, resample=Image.Resampling.BICUBIC, expand=True)
    story.alpha_composite(sticker, (110, 350))
    panel_y = 1305
    d = ImageDraw.Draw(story, 'RGBA')
    d.rounded_rectangle((70, panel_y, 1010, 1820), radius=56, fill=(247,244,239,245))
    ...
    for pt in pts:
        ...
        y += 84
    d.rounded_rectangle((160, 1698, 920, 1794), radius=36, fill=(255,92,45,255))
    d.text((540, 1745), 'CONFIRA NA SHOPEE', anchor='mm', font=font(42), fill=(255,255,255,255))
    ...
    prod = _hero_product(product_png, (880, 700))
    px = (1080 - prod.width) // 2
    py = 248
    _paste_product_with_shadow(feed, prod, px, py, 24)
    d.rounded_rectangle((70, 850, 1010, 1280), radius=50, fill=(247,244,239,245))
    tf = _fit_single_line(d, title, 800, 54, 28)
    d.text((540, 922), title, anchor='mm', font=tf, fill=(20,22,28,255))
    y = 1005
    for pt in pts:
        ...
        y += 68
    d.rounded_rectangle((225, 1188, 855, 1262), radius=28, fill=(255,92,45,255))
    d.text((540, 1225), 'VER NA SHOPEE', anchor='mm', font=font(34), fill='white')
...
def render(product, background, product_png, audio, captions_ass, work):
    ...
    fc = f"""[0:v]scale=1080:1920,zoompan=z='min(zoom+0.00028,1.085)':x='iw/2-(iw/zoom/2)+9*sin(on/42)':y='ih/2-(ih/zoom/2)+7*cos(on/53)':d=1:s=1080x1920:fps=30,eq=brightness=-0.03:saturation=1.12,vignette=PI/5[bg0];
[8:v]format=rgba,colorchannelmixer=aa=0.72[flare];
[bg0][flare]overlay=x='(W-w)/2+4*sin(t*0.85)':y='450+6*cos(t*0.75)':eval=frame[bg];
[1:v]format=rgba,scale=980:-1,fade=t=in:st=0:d=0.24:alpha=1[p];
[bg][p]overlay=x='(W-w)/2+10*sin(t*1.18)':y='if(lt(t,0.55),860-(t/0.55)*420,440+10*sin(t*1.2)+4*sin(t*2.6))':eval=frame[v1];
[7:v]format=rgba[shine];[shine]split=3[s1][s2][s3];
[v1][s1]overlay=x='-w+(t-0.70)/0.82*(W+w)':y=0:enable='between(t,0.70,1.52)':eval=frame[v1a];
[v1a][s2]overlay=x='-w+(t-7.70)/0.82*(W+w)':y=0:enable='between(t,7.70,8.52)':eval=frame[v1b];
[v1b][s3]overlay=x='-w+(t-13.90)/0.82*(W+w)':y=0:enable='between(t,13.90,14.72)':eval=frame[v1c];
[2:v]format=rgba[hook];[v1c][hook]overlay=x='(W-w)/2':y='if(lt(t,0.36),-h+(t/0.36)*(90+h),90)':enable='between(t,0,5.2)':eval=frame[v2];
[3:v]format=rgba[sticker];[v2][sticker]overlay=x='750+8*sin(t*1.6)':y='365+8*cos(t*1.3)':enable='between(t,0.9,6.8)':eval=frame[v3];
[4:v]format=rgba[arrow];[v3][arrow]overlay=x='85+8*sin(t*0.9)':y='650+6*cos(t*1.1)':enable='between(t,1.4,7.2)':eval=frame[v4];
[5:v]format=rgba[board];[v4][board]overlay=x='(W-w)/2':y='if(lt(t,6.7),H-(t-6.15)/0.55*(H-1180),1180)':enable='gte(t,6.15)':eval=frame[v5];
[6:v]format=rgba[cta];[v5][cta]overlay=x='(W-w)/2':y='if(lt(t,{end_cta+0.35:.3f}),H-(t-{end_cta:.3f})/0.35*(H-1660),1660)':enable='gte(t,{end_cta:.3f})':eval=frame[v6];
[v6]ass=captions.ass[v];
[9:a]volume=1.12[narr];
[10:a]volume=0.15[musicbed];
[narr][musicbed]amix=inputs=2:duration=first:dropout_transition=1.4,alimiter=limit=0.94[aout]
""".replace('\n','')

    output = work / 'anuncio_final.mp4'
    cmd = ['ffmpeg', '-y']
    for img in [bg_local, product_png, work/'hook.png', work/'sticker.png', work/'arrow.png', work/'board.png', work/'cta.png', work/'shine.png', work/'hero_flare.png']:
...
def main():
    if len(sys.argv)!=3:
        raise SystemExit('Uso: render_v8.py JOB_DIR OUTPUT_DIR')
    ...
    run(['edge-tts','--file',str(roteiro),'--voice',voice,'--rate=+8%','--pitch=+2Hz','--write-media',str(audio),'--write-subtitles',str(srt)])
    ...
    (output_dir/'copie_e_cole_na_live.txt').write_text(
        ...
        encoding='utf-8')
    print('[OK] Render V8 lúdico comercial concluído.',flush=True)
