...
[1:v]format=rgba,scale=860:-1,fade=t=in:st=0:d=0.24:alpha=1[p];
[bg][p]overlay=x='(W-w)/2+8*sin(t*1.1)':y='if(lt(t,0.55),910-(t/0.55)*580,330+8*sin(t*1.1)+4*sin(t*2.4))':eval=frame[v1];
...
[5:v]format=rgba[board];[v4][board]overlay=x='(W-w)/2':y='if(lt(t,5.9),H-(t-5.28)/0.62*(H-1320),1320)':enable='gte(t,5.28)':eval=frame[v5];
[6:v]format=rgba[cta];[v5][cta]overlay=x='(W-w)/2':y='if(lt(t,{end_cta+0.35:.3f}),H-(t-{end_cta:.3f})/0.35*(H-1732),1732)':enable='gte(t,{end_cta:.3f})':eval=frame[v6];
...
def main():
    if len(sys.argv)!=3:
        raise SystemExit('Uso: render_v13.py JOB_DIR OUTPUT_DIR')
    ...
    (output_dir/'copie_e_cole_na_live.txt').write_text(
        ...
        encoding='utf-8')
    print('[OK] Render V13 marketing concluído.',flush=True)
