...
def make_marketing_banners(product, background, product_png, output_dir):
    ...
    panel_y = 1450
    d = ImageDraw.Draw(story, 'RGBA')
    d.rounded_rectangle((70, panel_y, 1010, 1850), radius=56, fill=(247,244,239,245))
    ...
    for pt in pts:
        ...
        y += 72
    d.rounded_rectangle((160, 1738, 920, 1834), radius=36, fill=(255,92,45,255))
    cta_main = str(product.get('cta_main') or 'CONFIRA NA SHOPEE').upper()
    d.text((540, 1786), cta_main, anchor='mm', font=font(42), fill=(255,255,255,255))
...
def build_background_video(job_dir, fallback_background, duration, work):
    ...
    assets = [('video', p) for p in videos] + [('image', p) for p in images]
    seg_dir = work / 'segments'
    seg_dir.mkdir(parents=True, exist_ok=True)
    segments = []
    elapsed = 0.0
    idx = 0
    limit = max(10, len(assets) * 4)

    while elapsed < duration + 0.25 and idx < limit:
        kind, src = assets[idx % len(assets)]
        remaining = duration - elapsed
        seg_dur = 4.2 if kind == 'video' else 3.2
        if remaining < seg_dur:
            seg_dur = max(1.8, remaining + 0.2)
    ...
    try:
        run([
            'ffmpeg','-y','-f','concat','-safe','0','-i',str(concat_list),'-t',f'{duration:.3f}',
...
def render(job_dir, product, background, product_png, audio, captions_ass, work):
    ...
    fc = f"""[0:v]scale=1080:1920,setsar=1,eq=brightness=-0.03:saturation=1.10,vignette=PI/5[bg0];
[8:v]format=rgba,colorchannelmixer=aa=0.70[flare];
[bg0][flare]overlay=x='(W-w)/2+4*sin(t*0.85)':y='450+6*cos(t*0.75)':eval=frame[bg];
[1:v]format=rgba,scale=840:-1,fade=t=in:st=0:d=0.24:alpha=1[p];
[bg][p]overlay=x='(W-w)/2+8*sin(t*1.1)':y='if(lt(t,0.55),900-(t/0.55)*610,290+8*sin(t*1.1)+4*sin(t*2.4))':eval=frame[v1];
[9:v]format=rgba,scale=390:-1,fade=t=in:st=0.15:d=0.35:alpha=1[pres];
[v1][pres]overlay=x='24+6*sin(t*0.9)':y='965+5*cos(t*1.05)':enable='between(t,0.45,6.8)':eval=frame[v1p];
[7:v]format=rgba[shine];[shine]split=3[s1][s2][s3];
[v1p][s1]overlay=x='-w+(t-0.70)/0.82*(W+w)':y=0:enable='between(t,0.70,1.52)':eval=frame[v1a];
...
[2:v]format=rgba[hook];[v1c][hook]overlay=x='(W-w)/2':y='if(lt(t,0.36),-h+(t/0.36)*(90+h),90)':enable='between(t,0,4.8)':eval=frame[v2];
[3:v]format=rgba[sticker];[v2][sticker]overlay=x='748+8*sin(t*1.6)':y='344+8*cos(t*1.3)':enable='between(t,0.85,5.8)':eval=frame[v3];
[4:v]format=rgba[arrow];[v3][arrow]overlay=x='88+8*sin(t*0.9)':y='640+6*cos(t*1.1)':enable='between(t,1.1,6.2)':eval=frame[v4];
[5:v]format=rgba[board];[v4][board]overlay=x='(W-w)/2':y='if(lt(t,5.9),H-(t-5.28)/0.62*(H-1440),1440)':enable='gte(t,5.28)':eval=frame[v5];
[6:v]format=rgba[cta];[v5][cta]overlay=x='(W-w)/2':y='if(lt(t,{end_cta+0.35:.3f}),H-(t-{end_cta:.3f})/0.35*(H-1748),1748)':enable='gte(t,{end_cta:.3f})':eval=frame[v6];
[v6]ass=captions.ass[v];
[10:a]volume=1.12[narr];
[11:a]volume=0.15[musicbed];
...
""".replace('\n','')
    ...
    for img in [product_png, work/'hook.png', work/'sticker.png', work/'arrow.png', work/'board.png', work/'cta.png', work/'shine.png', work/'hero_flare.png', presenter_png]:
...
def main():
    if len(sys.argv)!=3:
        raise SystemExit('Uso: render_v14.py JOB_DIR OUTPUT_DIR')
    ...
    (output_dir/'copie_e_cole_na_live.txt').write_text(
        ...
        encoding='utf-8')
    print('[OK] Render V14 marketing concluído.',flush=True)
