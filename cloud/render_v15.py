...
[bg][p]overlay=x='(W-w)/2+8*sin(t*1.1)':y='if(lt(t,0.55),980-(t/0.55)*640,340+8*sin(t*1.1)+4*sin(t*2.4))':eval=frame[v1];
...
[5:v]format=rgba[board];[v4][board]overlay=x='(W-w)/2':y='if(lt(t,5.9),H-(t-5.28)/0.62*(H-1490),1490)':enable='gte(t,5.28)':eval=frame[v5];
...
def main():
    if len(sys.argv)!=3:
        raise SystemExit('Uso: render_v15.py JOB_DIR OUTPUT_DIR')
    ...
    (output_dir/'copie_e_cole_na_live.txt').write_text(
        ...
        encoding='utf-8')
    print('[OK] Render V15 marketing concluído.',flush=True)
