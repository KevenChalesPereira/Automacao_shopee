...
def make_marketing_banners(product, background, product_png, output_dir):
    ...
    for pt in pts:
        ...
        y += 84
    d.rounded_rectangle((160, 1712, 920, 1808), radius=36, fill=(255,92,45,255))
    cta_main = str(product.get('cta_main') or 'CONFIRA NA SHOPEE').upper()
    d.text((540, 1760), cta_main, anchor='mm', font=font(42), fill=(255,255,255,255))
    ...
    prod = _hero_product(product_png, (820, 620))
    px = (1080 - prod.width) // 2
    py = 262
...
[1:v]format=rgba,scale=930:-1,fade=t=in:st=0:d=0.24:alpha=1[p];
[bg][p]overlay=x='(W-w)/2+10*sin(t*1.18)':y='if(lt(t,0.55),860-(t/0.55)*420,390+10*sin(t*1.2)+4*sin(t*2.6))':eval=frame[v1];
[9:v]format=rgba,scale=420:-1,fade=t=in:st=0.15:d=0.35:alpha=1[pres];
[v1][pres]overlay=x='20+6*sin(t*0.9)':y='880+6*cos(t*1.1)':enable='between(t,0.45,6.8)':eval=frame[v1p];
...
[3:v]format=rgba[sticker];[v2][sticker]overlay=x='760+8*sin(t*1.6)':y='330+8*cos(t*1.3)':enable='between(t,0.85,5.8)':eval=frame[v3];
[4:v]format=rgba[arrow];[v3][arrow]overlay=x='90+8*sin(t*0.9)':y='650+6*cos(t*1.1)':enable='between(t,1.1,6.2)':eval=frame[v4];
[5:v]format=rgba[board];[v4][board]overlay=x='(W-w)/2':y='if(lt(t,5.8),H-(t-5.25)/0.55*(H-1260),1260)':enable='gte(t,5.25)':eval=frame[v5];
[6:v]format=rgba[cta];[v5][cta]overlay=x='(W-w)/2':y='if(lt(t,{end_cta+0.35:.3f}),H-(t-{end_cta:.3f})/0.35*(H-1692),1692)':enable='gte(t,{end_cta:.3f})':eval=frame[v6];
...
""".replace('\n','')
    ...
    cmd = ['ffmpeg', '-y']
    for img in [bg_local, product_png, work/'hook.png', work/'sticker.png', work/'arrow.png', work/'board.png', work/'cta.png', work/'shine.png', work/'hero_flare.png', presenter_png]:
...
def main():
    if len(sys.argv)!=3:
        raise SystemExit('Uso: render_v12.py JOB_DIR OUTPUT_DIR')
    ...
    (output_dir/'copie_e_cole_na_live.txt').write_text(
        ...
        encoding='utf-8')
    print('[OK] Render V12 marketing concluído.',flush=True)
