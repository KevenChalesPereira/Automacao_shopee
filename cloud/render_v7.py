...
def make_marketing_banners(product, background, product_png, output_dir):
    ...
    while len(pts) < 3: pts.append('CONFIRA')

    # Story / Shopee vertical 1080x1920
    story = _cover_background(background, (1080,1920))
    d = ImageDraw.Draw(story, 'RGBA')
    d.rounded_rectangle((60,70,1020,285), radius=42, fill=(8,11,18,185))
    hook_wrapped='\n'.join(textwrap.wrap(hook, width=28, break_long_words=False))
    hf=fit_font(d,hook_wrapped,870,52,30)
    d.multiline_text((540,175),hook_wrapped,anchor='mm',align='center',font=hf,fill=(255,255,255,255),spacing=5)

    prod=_hero_product(product_png,(930,1030)); px=(1080-prod.width)//2; py=300
    _paste_product_with_shadow(story,prod,px,py,30)

    panel_y=1330
    d.rounded_rectangle((70,panel_y,1010,1810),radius=54,fill=(248,245,238,238))
    tf=_fit_single_line(d,title,820,58,30)
    d.text((540,panel_y+78),title,anchor='mm',font=tf,fill=(22,24,28,255))
    y=panel_y+155
    for pt in pts:
        d.ellipse((130,y-12,164,y+22),fill=(39,146,83,255))
        d.text((147,y+4),'✓',anchor='mm',font=font(27),fill=(255,255,255,255))
        pf=_fit_single_line(d,str(pt).upper(),700,38,26)
        d.text((190,y+4),str(pt).upper(),anchor='lm',font=pf,fill=(28,31,36,255))
        y += 72
    d.rounded_rectangle((150,1685,930,1780),radius=36,fill=(255,92,45,255))
    d.text((540,1732),'CONFIRA NA SHOPEE',anchor='mm',font=font(43),fill=(255,255,255,255))
    story_path=output_dir/'banner_story.png'; story.convert('RGB').save(story_path,quality=94)

    # Feed 4:5 1080x1350
    feed=_cover_background(background,(1080,1350))
    d=ImageDraw.Draw(feed,'RGBA')
    d.rounded_rectangle((55,55,1025,225),radius=40,fill=(8,11,18,180))
    short_hook='\n'.join(textwrap.wrap(hook,width=31,break_long_words=False))
    hf=fit_font(d,short_hook,880,46,28)
    d.multiline_text((540,140),short_hook,anchor='mm',align='center',font=hf,fill=(255,255,255,255),spacing=4)
    prod=_hero_product(product_png,(820,720)); px=(1080-prod.width)//2; py=235
    _paste_product_with_shadow(feed,prod,px,py,24)
    d.rounded_rectangle((70,900,1010,1290),radius=46,fill=(248,245,238,240))
    tf=_fit_single_line(d,title,820,52,28); d.text((540,965),title,anchor='mm',font=tf,fill=(24,26,31,255))
    y=1040
    for pt in pts:
        d.ellipse((140,y-11,174,y+23),fill=(39,146,83,255)); d.text((157,y+5),'✓',anchor='mm',font=font(26),fill='white')
        d.text((202,y+5),str(pt).upper(),anchor='lm',font=_fit_single_line(d,str(pt).upper(),660,34,24),fill=(30,32,36,255)); y+=60
    d.rounded_rectangle((230,1210,850,1275),radius=28,fill=(255,92,45,255)); d.text((540,1242),'VER NA SHOPEE',anchor='mm',font=font(34),fill='white')
    feed_path=output_dir/'banner_feed.png'; feed.convert('RGB').save(feed_path,quality=94)

    # Minimal video cover: little text, strong product.
    cover=_cover_background(background,(1080,1920)); d=ImageDraw.Draw(cover,'RGBA')
    d.rounded_rectangle((75,85,1005,285),radius=44,fill=(8,11,18,178))
    ch='\n'.join(textwrap.wrap(hook,width=27,break_long_words=False)); cf=fit_font(d,ch,820,56,32)
    d.multiline_text((540,185),ch,anchor='mm',align='center',font=cf,fill='white',spacing=5)
    prod=_hero_product(product_png,(950,1180)); px=(1080-prod.width)//2; py=330; _paste_product_with_shadow(cover,prod,px,py,30)
    d.rounded_rectangle((120,1625,960,1765),radius=46,fill=(255,92,45,250)); d.text((540,1695),'ACHADO NA SHOPEE',anchor='mm',font=font(48),fill='white')
    cover_path=output_dir/'capa_video.png'; cover.convert('RGB').save(cover_path,quality=94)
...
[1:v]format=rgba,scale=920:-1,fade=t=in:st=0:d=0.26:alpha=1[p];
[bg][p]overlay=x='(W-w)/2+12*sin(t*1.20)':y='if(lt(t,0.52),800-(t/0.52)*405,395+15*sin(t*1.05))':eval=frame[v1];
...
def main():
    if len(sys.argv)!=3:
        raise SystemExit('Uso: render_v7.py JOB_DIR OUTPUT_DIR')
    ...
    (output_dir/'copie_e_cole_na_live.txt').write_text(
        ...
        encoding='utf-8')
    print('[OK] Render V7 comercial concluído.',flush=True)
